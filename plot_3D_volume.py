...
def plot_volume_isosurface(volume_file, box_name):

    ##################
    # data load / prep
    
    print(f">> loading serialized data (in file {volume_file}")

    # load the data
    x,y,r,m0,m = pickle.load(open(volume_file, 'rb'))
    if config['cut_top']:
    ...
    config_out_file = os.path.join(f"data.{box_name}", f"config.{config['set_name']}.json")
    ...
    for view in config['views']:
        ...
        if config['savefig']['save']:
            t0 = time()
            fig_name = os.path.join(f"data.{box_name}", 'frame_%4.4i.%s.%s' % (i, config['set_name'], config['savefig']['ext']))
            f.scene.save(fig_name)
            t1 = time()
...
def main():
    ...
    parser.add_argument("box_name",type=str,help="box_name used when running extract_volume_cartesian. These two scripts have to be run in the same directory")
    # parser.add_argument('volume_file', type=str, help='serialized data generated by an extract_volume program', default=f"volume.{config['box_name']}.pkl")
    args = parser.parse_args()
    
    box_name = args.box_name
    
    volume_file = os.path.join(f"data.{box_name}", f'volume.{box_name}.pkl')

    plot_volume_isosurface(volume_file = volume_file, box_name=args.box_name)
