...
"""
Plot the 3d model and add path computed with obspy from one event and several receivers.
"""
...
import geopy
from geopy.distance import geodesic # from a reference point, a distance and an azimuth give the other point
...
config = {
    ...
    # name of the exported volume (i.e. will look for file volume.{boxname}.{suffix}.pkl in directory $box_name)
    'box_name': 'island',    
    ...
    'views': [
        {'azimuth': 90.0, 'elevation': 40.0, 'distance': 220.0},
    ],
    ...
    # information for path plotting
    "paths" : True,
    "ev_lat" : 32.6,    # latitude in degrees
    "ev_lon" : 47.,     # longitude in degrees
    "ev_depth" : 12.,  # depth in km
    "phases" : ["S", "Sdiff"],    # phases
    "receiv_file" : "island/receivers.asc", # receiver file : 2 column lat lon, no header
    
    # ulvz plotting
    "ulvz" : True,
    "lat_ulvz" : 67.0,     # latitude in degrees
    "lon_ulvz" : -17.0,     # longitude in degrees
    "height_ulvz" : 20.0,   # height above CMB in km
    "radius_ulvz" : 400.0,   # radius of the cylinder in km
}
...
def plot_hotspots(m, fac, xlims, ylims, r_min, r_plot = None, **kwargs):
    ...
    for x, y in zip(xs, ys):
        ...
        if x >= xlims[0] and x <= xlims[1] and y >= ylims[0] and y <= ylims[1]:
            ...
            m.plot3d([x,x],[y,y],[r_min / fac,r_plot / fac], color=color, tube_radius=tube_radius)
...
def plot_paths(m, fac):
    
    # configuration
    tube_radius = 1e-1
    alpha = 1.0
    
    # reading stations information
    stations_latlon = np.loadtxt(config["receiv_file"])
    
    # computing ray path
    evlat,evlon,evdepth = config["ev_lat"], config["ev_lon"], config["ev_depth"]
    
    # plot source
    m.points3d([evlon], [evlat], [(RN-evdepth) / fac], mode="sphere", scale_factor=2, color=(1,0,0))
    
    print(">> Computing ray paths")
    
    for stlat, stlon in stations_latlon:

        # return path in (distance-depth)
        arrivals = model.get_ray_paths_geo(
            evdepth, evlat, evlon, stlat, stlon, 
            phase_list=config["phases"],
            resample=True)
        ...
        for arr in arrivals:
            ...
            r_path = RN - arr.path['depth'] 
            x_path = arr.path['lon']
            y_path = arr.path['lat']
                        
            m.plot3d(x_path, y_path, r_path / fac, opacity=alpha, tube_radius=tube_radius, color=(1,1,1))
 

def plot_ulvz(m, fac):
    
    """PLot cylindrical ULVZ, defined in the config dictionary
    """
    
    N = 20
    
    z0 = 3480./fac
    y0,x0,h,r = config["lat_ulvz"], config["lon_ulvz"],config["height_ulvz"]/fac,config["radius_ulvz"]
    
    print(y0,x0,h,r)
    
    alpha = np.linspace(0.0, 360.0, N)

    # # pour r donné en °
    # x_list = x0 + r * np.cos(alpha)
    # y_list = y0 + r * np.sin(alpha)
    
    # pour r donné en km
    x_list = []
    y_list = []
    origin = geopy.Point(y0,x0)
    for az in alpha:
        point = geodesic(kilometers=r).destination(origin, bearing=az)
        x_list.append(point.longitude)
        y_list.append(point.latitude)
    ...
    for i in range(N):
        # side triangles
        triangles.append([i,N+i,(i+1)%N])
        triangles.append([N+i,N+(i+1)%N,(i+1)%N])
        # base and summit triangles
        triangles.append([i,(i+1)%N,2*N])
        triangles.append([N+i,N+(i+1)%N,2*N+1])

    triangles = np.array(triangles)
    m.triangular_mesh(points[:,0], points[:,1], points[:,2], triangles, color=(240/255,40/255,240/255))
...
def plot_volume_isosurface():
    ...
    # load the data
    x,y,r,m0,m = pickle.load(open('%s/volume.%s.pkl' % (config['box_name'],config['box_name']), 'rb'))
    ...
    with open('%s/config.%s.json' % (config['box_name'],config['set_name']), 'w') as fp:
